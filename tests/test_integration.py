...
class TestSSHMCPRealIntegration:
    ...
    def test_host_test_exists_in_config(self):
        ...
        if "test" not in hosts:
            pytest.skip(
                "Host 'test' not found in SSH config. Add a host named 'test' to run integration tests."
            )
    ...
    def test_ssh_config_loading(self):
        ...
        ) or config.config_file_path == os.path.expanduser("~/.ssh/config")
    ...
    async def test_execute_command_on_test_host(self):
        ...
        if "SUCCESS" in result_text:
            ...
            assert (
                "Linux" in result_text or "EXIT" in result_text
            )  # uname -s output or exit code
        else:
    ...
    async def test_execute_command_filesystem_check(self):
        ...
        result = await mcp.call_tool(
            "execute_ssh_command",
            {"hostname": "test", "command": "pwd && ls -la / | head -5"},
        )
        result_text = str(result)
        ...
        if "SUCCESS" in result_text:
            ...
            assert (
                "bin" in result_text
                or "etc" in result_text
                or "home" in result_text
                or "/" in result_text
            )  # Basic Linux filesystem check
